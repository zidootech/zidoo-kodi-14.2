    ...
    case TMSG_PAUSE:
      {
        ...
        {  
            CLog::Log(LOGNOTICE, "App pause and media is playing, pause it first");
            g_application.m_pPlayer->Pause();
            //g_application.StopPlaying();
         }
        
         g_application.SetAppState(0);
      }
      break;
    case TMSG_RESUME:
      {
        g_application.SetAppState(1);
        /*
...
void CApplicationMessenger::Pause()
{
  ThreadMessage tMsg = {TMSG_PAUSE};
  SendMessage(tMsg, true);
}

void CApplicationMessenger::Resume()
{
  ThreadMessage tMsg = {TMSG_RESUME};
  SendMessage(tMsg);
}
