...
bool CRepository::Parse(const DirInfo& dir, VECADDONS &result)
{
      ...
      CAddonMgr::Get().AddonsFromRepoXML(doc.RootElement(), result))
  {
    for (IVECADDONS i = result.begin(); i != result.end(); ++i)
    {
      ...
      {
        string file;
        if (!addon->UserDir().empty())
        {
          CLog::Log(LOGDEBUG, "CRepository : userdir  = %s", addon->UserDir().c_str());
          file = StringUtils::Format("%s/%s/%s-%s.zip", addon->UserDir().c_str(), addon->ID().c_str(), addon->ID().c_str(), addon->Version().asString().c_str());
          addon->Props().path = URIUtils::AddFileToFolder(dir.datadir,file);
          SET_IF_NOT_EMPTY(addon->Props().icon,URIUtils::AddFileToFolder(dir.datadir,addon->UserDir() + "/" + addon->ID()+"/icon.png"))
          file = StringUtils::Format("%s/%s/changelog-%s.txt", addon->UserDir().c_str(), addon->ID().c_str(), addon->Version().asString().c_str());
          SET_IF_NOT_EMPTY(addon->Props().changelog,URIUtils::AddFileToFolder(dir.datadir,file))
          SET_IF_NOT_EMPTY(addon->Props().fanart,URIUtils::AddFileToFolder(dir.datadir,addon->UserDir() + "/" + addon->ID()+"/fanart.jpg"))
        } else {
          file = StringUtils::Format("%s/%s-%s.zip", addon->ID().c_str(), addon->ID().c_str(), addon->Version().asString().c_str());        
          addon->Props().path = URIUtils::AddFileToFolder(dir.datadir,file);
      ...
      else
      {
        ...
        {
          CLog::Log(LOGDEBUG, "CRepository : userdir  = %s", addon->UserDir().c_str());
          addon->Props().path = URIUtils::AddFileToFolder(dir.datadir,addon->UserDir() + "/" + addon->ID()+"/");
          SET_IF_NOT_EMPTY(addon->Props().icon,URIUtils::AddFileToFolder(dir.datadir,addon->UserDir() + "/" + addon->ID()+"/icon.png"))
          SET_IF_NOT_EMPTY(addon->Props().changelog,URIUtils::AddFileToFolder(dir.datadir,addon->UserDir() + "/" + addon->ID()+"/changelog.txt"))
          SET_IF_NOT_EMPTY(addon->Props().fanart,URIUtils::AddFileToFolder(dir.datadir,addon->UserDir() + "/" + addon->ID()+"/fanart.jpg"))
        } else {
