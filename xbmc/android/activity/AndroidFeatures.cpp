...
int  CAndroidFeatures::GetAuthentication()
{
  static int sCode = -1;
  ...
  if (sCode == -1)
  {
    ...
    jclass jcOsBuild = jenv->FindClass("android/os/Build$AUTHENTICATION");
    if (jcOsBuild == NULL) 
    {
      //CLog::Log(LOGERROR, "%s: Error getting class android.os.Build.AUTHENTICATION", __PRETTY_FUNCTION__);
      return sCode;
    }

    jint icode = jenv->GetStaticIntField(jcOsBuild, jenv->GetStaticFieldID(jcOsBuild, "CODE", "I"));
    //CLog::Log(LOGDEBUG, "%s: android.os.Build.AUTHENTICATION %d", __PRETTY_FUNCTION__, (int)icode);
