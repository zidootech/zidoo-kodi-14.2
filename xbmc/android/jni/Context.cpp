...
jhobject CJNIContext::m_Surface(0);
bool CJNIContext::m_surface_change(false);
...
void CJNIContext::_SetVideoPlaySurface(JNIEnv *env, jobject context, jobject surface)
{
	(void)env;
  	(void)context;	
    //__android_log_print(ANDROID_LOG_DEBUG, "Kodi", "_SetVideoPlaySurface");
    if (surface != NULL) {
      //__android_log_print(ANDROID_LOG_DEBUG, "Kodi", "_SetVideoPlaySurface -- 0");
	  m_Surface = jhobject(surface);
	  m_Surface.setGlobal();
    } 
    else {
      //__android_log_print(ANDROID_LOG_DEBUG, "Kodi", "_SetVideoPlaySurface -- 1");
      m_Surface.reset();
    }

    m_surface_change = true;
}

jni::jhobject CJNIContext::getSurface()
{
	return m_Surface;
}

void CJNIContext::setIsSurfaceChange(bool change)
{
  //__android_log_print(ANDROID_LOG_DEBUG, "Kodi", "setIsSurfaceChange(%d)", change ? 1 : 0);
  m_surface_change  = change;
}

bool  CJNIContext::isSurfaceChange()
{
    //__android_log_print(ANDROID_LOG_DEBUG, "Kodi", "isSurfaceChange(%d)", m_surface_change ? 1 : 0);
	return m_surface_change;
}