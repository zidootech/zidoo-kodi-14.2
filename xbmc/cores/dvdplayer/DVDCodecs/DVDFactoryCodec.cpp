...
#include "Video/DVDVideoCodecAW.h"
#endif
...
#if defined(TARGET_ANDROID)
    if (!hint.software && CSettings::Get().GetBool("videoplayer.useawcodec"))
    {
      CLog::Log(LOGINFO, "CDVDVideoCodecAW Video Decoder...");
      if ( (pCodec = OpenCodec(new CDVDVideoCodecAW(), hint, options)) ) return pCodec;
    }
