...
void CDVDPlayer::Process()
{
  ...
  while (!m_bAbortRequest)
  {
    ...
    // check video information is ready or not(ready means valid)
    // if ready send ready to gui to check if adjust system refresh to match video
    if (!m_videoinfo_ready) 
    {
        SPlayerVideoStreamInfo video;
        GetVideoStreamInfo(video);
        if (video.width != 0 && video.height != 0 && ((int)video.framerate != 0)) 
        {  
            if (!vidoeInfoUpdated) 
            {
                g_dataCacheCore.SignalVideoInfoChange();
                g_infoManager.UpdateAVInfo();
                vidoeInfoUpdated = true;
            }
            if (m_callback.OnPlayBackVideoInfoReady())
                m_videoinfo_ready = true;
        }        
    }
  }
}
...
bool CDVDPlayer::OpenVideoStream(CDVDStreamInfo& hint, bool reset)
{
  ...
  if (m_pDemuxer != NULL) {
    m_dvdPlayerVideo->SetStreamInputFormat(m_pDemuxer->GetStreamInputFormat());
  }
  return true;
...
void CDVDPlayer::GetVideoStreamInfo(SPlayerVideoStreamInfo &info)
{
  ...
  info.framerate = m_dvdPlayerVideo->GetVideoFrameRate();
}
...
void CDVDPlayer::SetAppState(int state)
{
    CLog::Log(LOGDEBUG, "%s - Enter", __FUNCTION__);
    m_dvdPlayerVideo->SetAppState(state);
}