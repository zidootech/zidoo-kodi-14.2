...
bool CDVDPlayerVideo::OpenStream( CDVDStreamInfo &hint )
{
  ...
  if (m_pVideoCodec) {
    const char * name = m_pVideoCodec->GetName();
    if (name && strncmp(name, "awcodec", strlen("awcodec")) == 0) {
      CLog::Log(LOGNOTICE, "awcode has created, delete it first");
...
void CDVDPlayerVideo::Process()
{
  ...
  while (!m_bStop)
  {
    ...
    if (pMsg->IsType(CDVDMsg::PLAYER_SET_APP_STATE))
    {
        int state = static_cast<CDVDMsgInt*>(pMsg)->m_value;
        if (m_pVideoCodec)
            m_pVideoCodec->SetAppState(state);
        m_waitpause = false;
    }
...
double CDVDPlayerVideo::GetVideoFrameRate()
{
  if (m_hints.rfpsrate > 0 && m_hints.rfpsscale != 0)
  {
    return (m_hints.rfpsrate * 1000 / m_hints.rfpsscale); 
  }
  else if (m_hints.fpsrate > 0 && m_hints.fpsscale != 0)
  {
    return (m_hints.fpsrate * 1000 / m_hints.fpsscale); 
  }
  else 
    return 0.0;
}
...
void CDVDPlayerVideo::SetAppState(int state)
{
  if (0/*m_messageQueue.IsInited()*/) {
    CLog::Log(LOGNOTICE, "SetAppState: messag");
    int count = 0;
    m_waitpause = true;
    m_messageQueue.Put(new CDVDMsgInt(CDVDMsg::PLAYER_SET_APP_STATE, state), 1);
    while(m_waitpause && count++ < 3) {
        sleep(1);
    }
  }
  else
  {
    CLog::Log(LOGNOTICE, "SetAppState: directly");
    if (m_pVideoCodec)
        m_pVideoCodec->SetAppState(state);
  }
}

void CDVDPlayerVideo::SetStreamInputFormat(const char *name)
{
    if (m_pVideoCodec) {
        m_pVideoCodec->SetStreamInputFormat(name);
    }
}
