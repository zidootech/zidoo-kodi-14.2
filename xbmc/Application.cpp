...
bool CApplication::OnPlayBackVideoInfoReady()
{
  CSingleLock lock(m_playStateMutex);
  CLog::LogF(LOGDEBUG, "play state was %d, starting %d", m_ePlayState, m_bPlaybackStarting);
  if (m_bPlaybackStarting || m_ePlayState != PLAY_STATE_PLAYING)
    return false;

  CGUIMessage msg(GUI_MSG_PLAYBACK_VIDEOIFNO_READY, 0, 0);
  g_windowManager.SendThreadMessage(msg);
...
void CApplication::SetAppState(int state)
{
    m_pPlayer->SetAppState(state);
}